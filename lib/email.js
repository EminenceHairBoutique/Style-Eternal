...
/** Shipped notification with tracking. Sent by api/admin/orders.js on "ship". */
export async function sendOrderShippedEmail({
  ...
  const carrierLabel = trackingCarrier ? escapeHtml(trackingCarrier.toUpperCase()) : "Carrier";

  const bodyHtml = `
    <p style="margin:0 0 14px;">Your order is on the way.</p>
    ...
    ${
      trackingNumber
        ? `
    <p style="margin:18px 0 4px;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:#A08A4A;">${carrierLabel} tracking</p>
    <p style="margin:0 0 18px;font-size:15px;color:#0A0A0A;letter-spacing:0.04em;">${escapeHtml(trackingNumber)}</p>`
        : ""
    }
    ...
    ${
      trackingUrl
        ? `
    <table role="presentation" cellpadding="0" cellspacing="0" style="margin:10px auto 8px;">
      <tr>
        <td style="background:#0A0A0A;">
          <a href="${escapeHtml(trackingUrl)}" style="display:inline-block;padding:14px 34px;font-family:Arial,Helvetica,sans-serif;font-size:12px;letter-spacing:0.25em;text-transform:uppercase;color:#E8E4DE;text-decoration:none;">Track your order</a>
        </td>
      </tr>
    </table>`
        : ""
...
/** Digital gift card delivery — one email per issued code (webhook). */
export async function sendGiftCardEmail({ to, code, amountCents }) {
  if (!to || !code) return null;

  const bodyHtml = `
    <p style="margin:0 0 14px;">Your Style Eternal gift card is ready. Share the code below — it redeems as store credit toward anything in the catalog and never expires.</p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:14px 0;">
      <tr>
        <td align="center" style="background:#0A0A0A;padding:26px 20px;">
          <p style="margin:0 0 6px;font-family:Arial,Helvetica,sans-serif;font-size:11px;letter-spacing:0.3em;text-transform:uppercase;color:#A08A4A;">Gift card</p>
          <p style="margin:0 0 10px;font-family:Georgia,'Times New Roman',serif;font-size:30px;color:#E8E4DE;">${usd((amountCents || 0) / 100)}</p>
          <p style="margin:0;font-family:'Courier New',monospace;font-size:18px;letter-spacing:0.12em;color:#E8E4DE;">${escapeHtml(code)}</p>
        </td>
      </tr>
    </table>

    <p style="margin:0;color:#6B6B6B;font-size:13px;">
      To redeem: sign in at shopstyleeternal.com → Account → Redeem gift card.
  ...
  return requireResend().emails.send({
    ...
    to,
    reply_to: SUPPORT,
    subject: `Your ${usd((amountCents || 0) / 100)} Style Eternal gift card`,
    html: renderShell({
      eyebrow: "Gift card",
...
/** Return lifecycle notifications. Sent by api/admin/orders.js on status changes. */
export async function sendReturnStatusEmail({ to, orderNumber, status, adminNotes }) {
  if (!to) return null;
  ...
  const copy = {
    requested: {
      ...
      body: "Our team reviews return requests within 1–2 business days. You'll hear from us as soon as it's approved.",
    },
    approved: {
      subject: `Return approved — ${orderNumber}`,
      eyebrow: "Return approved",
      heading: "Your return is approved.",
      body: "Pack the unworn items with their tags and ship them back within 14 days. Reply to this email if you need a label or have any questions.",
    },
    rejected: {
      subject: `Return update — ${orderNumber}`,
      eyebrow: "Return update",
      heading: "About your return request.",
      body: "We couldn't approve this return request. Reply to this email and our team will help find a resolution.",
    },
    received: {
      subject: `Return received — ${orderNumber}`,
      eyebrow: "Return received",
      heading: "Your return arrived.",
      body: "We're inspecting the items now. Refunds are issued to the original payment method within 3–5 business days of inspection.",
    },
    refunded: {
      subject: `Refund issued — ${orderNumber}`,
      eyebrow: "Refunded",
      heading: "Your refund is on its way.",
      body: "The refund has been issued to your original payment method. Depending on your bank, it can take 5–10 business days to appear.",
    },
  }[status];
  ...
  const bodyHtml = `
    <p style="margin:0 0 14px;">${copy.body}</p>
    <p style="margin:0 0 4px;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;color:#A08A4A;">Order number</p>
    <p style="margin:0 0 18px;font-size:18px;color:#0A0A0A;letter-spacing:0.05em;">${escapeHtml(orderNumber)}</p>
    ${
      adminNotes
        ? `<p style="margin:10px 0 0;padding:12px 14px;background:#F5F2EC;border:1px solid #E8E4DE;color:#3A3A3A;">${escapeHtml(adminNotes)}</p>`
        : ""
    }`;
  ...
  return requireResend().emails.send({
    ...
    html: renderShell({ eyebrow: copy.eyebrow, heading: copy.heading, bodyHtml }),
  });
}
