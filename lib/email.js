...
export async function sendBroadcastEmail({ to, subject, html }) {
  return requireResend().emails.send({
    from: "Style Eternal <news@styleeternal.com>",
    to,
    reply_to: "support@styleeternal.com",
    subject,
    html,
  });
}
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px;">
        ...
        <p style="font-size: 12px; color: #666;">
          Style Eternal · Premium quality · Built to last
        </p>
      </div>
    `,
  });
...
export async function sendConciergeRequestEmail({
  ...
  return requireResend().emails.send({
    from: "Style Eternal <concierge@styleeternal.com>",
    to,
    reply_to: payload?.email || "support@styleeternal.com",
    subject,
